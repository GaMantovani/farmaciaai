...
function gerarSlugFarmacia(f) {
  const bairro = f.bairro && f.bairro !== f.cidade ? norm(f.bairro) : ''
  const nome = norm(f.nome)
  return (bairro ? bairro + '-' + nome : nome).substring(0, 100)
}
...
export default function FarmaciaPage({ farmacia, outrasNaCidade }) {
  ...
  const cidadeSlug = norm(farmacia.cidade)
  const estadoSlug = farmacia.estado ? farmacia.estado.toLowerCase() : ''
  ...
  const schema = {
    ...
    ...(farmacia.avaliacao ? { "aggregateRating": { "@type": "AggregateRating", "ratingValue": farmacia.avaliacao, "bestRating": "5", "ratingCount": "10" } } : {})
  }
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={`${farmacia.nome} em ${farmacia.bairro}, ${farmacia.cidade}, ${farmacia.estado}. Endereço: ${farmacia.endereco}. Veja no mapa e compare preços.`} />
        <link rel="canonical" href={`https://farmaciaai.com.br/farmacia/${cidadeSlug}/${gerarSlugFarmacia(farmacia)}`} />
      ...
      <div style={{ background:'#111',padding:'40px 20px 48px' }}>
        <div style={{ maxWidth:900,margin:'0 auto' }}>
          ...
          <div style={{ display:'flex',alignItems:'flex-start',gap:16,flexWrap:'wrap' }}>
            <div style={{ width:52,height:52,borderRadius:12,background:rede.cor||'#ff4500',display:'flex',alignItems:'center',justifyContent:'center',fontSize:26,flexShrink:0 }}>💊</div>
            <div style={{ flex:1 }}>
              <h1 style={{ fontFamily:"'DM Serif Display',serif",fontSize:'clamp(22px,4vw,38px)',color:'#fff',lineHeight:1.15,marginBottom:8 }}>
                {farmacia.nome} <em style={{ color:'rgba(255,255,255,.4)',fontStyle:'italic',fontSize:'65%' }}>em {farmacia.bairro}, {farmacia.cidade}</em>
              </h1>
              <p style={{ fontSize:14,color:'rgba(255,255,255,.5)',marginBottom:10 }}>{farmacia.endereco}</p>
              {farmacia.avaliacao && <div style={{ display:'inline-flex',alignItems:'center',gap:4,background:'rgba(255,255,255,.1)',padding:'3px 10px',borderRadius:100,fontSize:12,color:'#fbbf24',marginBottom:12 }}>★ {farmacia.avaliacao} de avaliação</div>}
              <div style={{ display:'flex',gap:8,flexWrap:'wrap',marginTop:8 }}>
                <a href={`https://www.google.com/maps/place/?q=place_id:${farmacia.place_id}`} target="_blank" rel="noopener noreferrer"
                  style={{ display:'inline-flex',alignItems:'center',gap:6,background:'rgba(255,255,255,.1)',color:'#fff',padding:'7px 14px',borderRadius:8,fontSize:13,fontWeight:500 }}>📍 Ver no mapa</a>
                {rede.site && <a href={`https://www.${rede.site}`} target="_blank" rel="noopener noreferrer nofollow"
                  style={{ display:'inline-flex',alignItems:'center',gap:6,background:OG,color:'#fff',padding:'7px 14px',borderRadius:8,fontSize:13,fontWeight:600 }}>🛒 Site oficial</a>}
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{ maxWidth:900,margin:'0 auto',padding:'36px 20px 64px' }}>
        <div style={{ marginBottom:36 }}>
          <h2 style={{ fontSize:17,fontWeight:700,color:'#111',marginBottom:14 }}>Onde fica a {farmacia.nome} em {farmacia.bairro}?</h2>
          <div style={{ borderRadius:14,overflow:'hidden',border:'1px solid #f0f0f0',boxShadow:'0 2px 10px rgba(0,0,0,.06)' }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(170px,1fr))',gap:12,marginBottom:36 }}>
          {[['📍','Endereço',farmacia.endereco],['🏙️','Bairro',farmacia.bairro],['🏢','Cidade',`${farmacia.cidade} - ${farmacia.estado}`],['⭐','Avaliação',farmacia.avaliacao?`${farmacia.avaliacao} / 5`:'—']].map(([icon,label,value])=>(
            <div key={label} style={{ background:'#fff',border:'1px solid #f0f0f0',borderRadius:12,padding:'14px 16px' }}>
        ...
        <div style={{ background:'linear-gradient(135deg,#fff8f5,#fff3ee)',border:'1px solid #ffd4be',borderRadius:18,padding:'24px 28px',marginBottom:36 }}>
          <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:14 }}>
            <div>
              <div style={{ fontSize:11,fontWeight:700,color:'#ff4500',textTransform:'uppercase',letterSpacing:'.08em',marginBottom:6 }}>Economize na farmácia</div>
              <h2 style={{ fontFamily:"'DM Serif Display',serif",fontSize:20,color:'#111',marginBottom:6 }}>Compare preços em {farmacia.cidade}</h2>
              <p style={{ fontSize:13,color:'#666' }}>Veja se tem mais barato. Gratuito e em tempo real.</p>
            </div>
            <Link href="/" style={{ display:'inline-flex',alignItems:'center',gap:8,background:OG,color:'#fff',padding:'12px 24px',borderRadius:12,fontSize:14,fontWeight:700,whiteSpace:'nowrap',boxShadow:'0 4px 14px rgba(255,69,0,.25)' }}>
              Comparar preços →
            </Link>
          </div>
        </div>
        ...
        <div style={{ marginBottom:36 }}>
          <h2 style={{ fontSize:17,fontWeight:700,color:'#111',marginBottom:14 }}>Remédios disponíveis na {farmacia.nome}</h2>
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))',gap:10 }}>
            {REMEDIOS.slice(0,8).map(r=>(
              <Link key={r.slug} href={`/?q=${encodeURIComponent(r.principio)}`}
                style={{ background:'#fff',border:'1px solid #f0f0f0',borderRadius:11,padding:'11px 14px',display:'flex',justifyContent:'space-between',alignItems:'center',transition:'all .15s' }}
          ...
          <div style={{ marginBottom:36 }}>
            ...
            <div style={{ display:'flex',flexWrap:'wrap',gap:8 }}>
              {outrasNaCidade.map(f=>(
                <Link key={f.place_id} href={`/farmacia/${norm(f.cidade)}/${gerarSlugFarmacia(f)}`}
        ...
        <div>
          ...
          {[
            [`Onde fica a ${farmacia.nome} no ${farmacia.bairro}?`, `A ${farmacia.nome} está em ${farmacia.endereco}, ${farmacia.bairro}, ${farmacia.cidade} - ${farmacia.estado}.`],
            [`Qual o horário da ${farmacia.nome}?`, `Consulte o Google Maps clicando em "Ver no mapa" acima para ver o horário atualizado.`],
            [`Como comparar preços na ${farmacia.nome}?`, `Use o campo "Compare preços" no topo desta página ou acesse a homepage do FarmáciaAí.`],
          ].map(([q,a],i)=>(
            <details key={i} style={{ border:'1px solid #f0f0f0',borderRadius:10,marginBottom:8,overflow:'hidden' }}>
              <summary style={{ padding:'13px 16px',fontSize:14,fontWeight:500,color:'#222',cursor:'pointer',display:'flex',justifyContent:'space-between',alignItems:'center' }}>
                {q} <span style={{ color:'#aaa',flexShrink:0,marginLeft:8 }}>+</span>
              </summary>
              <div style={{ padding:'0 16px 13px',fontSize:14,color:'#666',lineHeight:1.7 }}>{a}</div>
            </details>
          ))}
        </div>
      </div>
...
export async function getStaticPaths() {
  const paths = TODAS
    .filter(f => f && f.cidade && f.nome && f.cidade.trim() && f.nome.trim())
    .map(f => ({
      params: {
        cidade: norm(f.cidade) || 'brasil',
        slug: gerarSlugFarmacia(f) || norm(f.nome) || f.place_id,
      }
    }))
    .filter(p => p.params.cidade && p.params.slug)
  return { paths, fallback: 'blocking' }
}

export async function getStaticProps({ params }) {
  const farmacia = TODAS.find(f =>
    norm(f.cidade) === params.cidade &&
    (gerarSlugFarmacia(f) === params.slug || norm(f.nome) === params.slug)
  )
  if (!farmacia) return { notFound: true }
  const outrasNaCidade = TODAS.filter(f => f.cidade === farmacia.cidade && f.place_id !== farmacia.place_id).slice(0,10)
  return { props: { farmacia, outrasNaCidade }, revalidate: 86400 }
}
