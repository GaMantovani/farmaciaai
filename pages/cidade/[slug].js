...
function CTABusca({ farmacia }) {
  ...
  return (
    <div style={{ position:'sticky',top:62,zIndex:200,background:'#fff',borderBottom:'2px solid #ffe0cc',padding:'10px 20px',boxShadow:'0 2px 12px rgba(255,90,0,.12)' }}>
      <div style={{ maxWidth:900,margin:'0 auto',display:'flex',alignItems:'center',gap:10,flexWrap:'wrap' }}>
        ...
        <div style={{ flex:1,display:'flex',gap:8,minWidth:200 }}>
          ...
          <a href={remedio.trim() ? '/?q='+encodeURIComponent(remedio) : '/'}
            style={{ height:36,padding:'0 16px',background:'linear-gradient(135deg,#ff6b1a,#ff4500)',color:'#fff',borderRadius:8,fontSize:13,fontWeight:700,display:'flex',alignItems:'center',whiteSpace:'nowrap',textDecoration:'none' }}>
            Comparar →
          </a>
        </div>
      </div>
    ...
    <>
          ...
          <div style={{ display:'flex',alignItems:'flex-start',gap:16,flexWrap:'wrap' }}>
            ...
            <div style={{ flex:1 }}>
              <h1 style={{ fontFamily:"'DM Serif Display',serif",fontSize:'clamp(24px,4vw,40px)',color:'#fff',lineHeight:1.15,marginBottom:8 }}>
                {farmacia.nome}
                <br/><em style={{ color:'rgba(255,255,255,.4)',fontStyle:'italic',fontSize:'65%' }}>em {farmacia.bairro}, {farmacia.cidade}</em>
              </h1>
      ...
      <div style={{ maxWidth:900,margin:'0 auto',padding:'40px 20px 72px' }}>
        <div style={{ marginBottom:40 }}>
          <h2 style={{ fontSize:18,fontWeight:700,color:'#111',marginBottom:16 }}>Onde fica a {farmacia.nome} em {farmacia.bairro}?</h2>
        ...
        <div style={{ background:'linear-gradient(135deg,#fff8f5,#fff3ee)',border:'1px solid #ffd4be',borderRadius:20,padding:'28px 32px',marginBottom:40 }}>
          <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:16 }}>
            <div>
              ...
              <h2 style={{ fontFamily:"'DM Serif Display',serif",fontSize:22,color:'#111',marginBottom:8 }}>Compare preços antes de comprar em {farmacia.cidade}</h2>
        ...
        <div>
          ...
          {[
            [`Onde fica a ${farmacia.nome} no ${farmacia.bairro}?`, `A ${farmacia.nome} está localizada em ${farmacia.endereco}, no bairro ${farmacia.bairro}, em ${farmacia.cidade}, ${farmacia.estado}.`],
            [`Qual o horário de funcionamento da ${farmacia.nome}?`, `Para verificar o horário atualizado, consulte o Google Maps clicando em "Ver no Google Maps" acima ou entre em contato com a unidade.`],
            [`Como chegar na ${farmacia.nome} em ${farmacia.cidade}?`, `Clique em "Ver no Google Maps" para obter as melhores rotas até a ${farmacia.nome} em ${farmacia.bairro}, ${farmacia.cidade}.`],
            [`Posso comparar preços de remédios na ${farmacia.nome}?`, `Sim! Use o FarmáciaAí para comparar preços de qualquer remédio em ${farmacia.cidade}. Gratuito e em tempo real.`],
          ].map(([q,a],i) => (
...
export async function getStaticPaths() {
  const paths = TODAS.map(f => ({
    params: {
      cidade: norm(f.cidade),
      slug: gerarSlugFarmacia(f) || norm(f.nome),
    }
